...
var FilterModal = Modal.extend({
    ...
    expression_text: function() {
        ...
        if (this.expressionType == "Order") {
            c += "<tr><td class='col1'>Sort Type: <select id='fun'><option value='ASC'>Ascendente</option><option value='BASC'>Ascendente ignorando hierarquia</option><option value='DESC'>Descendente</option><option value='BDESC'>Descendente quebrando hierarquia</option> </select></td></tr>" 
        }
        c += "<tr><td class='col1'>Expressão em MDX para a função: " + this.expressionType + "</td></tr>"
    ...
    initialize: function(args) {
        ...
        _.extend(this.options, {
            title: "Customização tipo " + this.expressionType + " para o eixo " + this.axis
        });
    ...
    save: function( event ) {
        ...
        if (typeof this.expression == "undefined" || !this.expression || this.expression == "") {
            alert_msg += "Você deve definir uma expressão em MDX para a função " + this.expressionType + "!";
            alert(alert_msg);
        } else {
    ...
    error: function() {
        $(this.el).find('dialog_body')
            .html("Não foi possível realizar a operação");
    }
